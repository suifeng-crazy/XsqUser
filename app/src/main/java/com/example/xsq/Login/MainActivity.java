...
public class MainActivity extends BaseActivity {
    //  mILonginMath 是登录方法， 1代表用户名密码登录， 2为 短信验证码登录
    // i 代表 执行哪个 runable ， 做区分用即可。
    //  mIGetCodeMath 代表 是因为 登录获取验证码  0，还是注册获取验证码  1。
    int mIi = 0, mIResStatus = 3, mILonginMath = 1, mIGetCodeMath = 0;
    // 获取验证码是否成功      注册验证码
    public Boolean mBoIsGetTrue, mBoIsGetRegTrue;
    ...
    Runnable mRunnable = new Runnable() {
        ...
        public void run() {
            try {
                if (mIi == 0) {
                    if (mILonginMath == 1) {
                        mIResStatus = PostParma.getToken("_a=" + userName, "_p=" + password);
                    } else {
                        mIResStatus = PostParma.getToken("_a=" + userName, "_c=" + login_phoneCodeI.getText().toString());
                    }
                    if(mIResStatus==0){
...
//                    mHandler.sendEmptyMessage(STATUS_CHECK);
                    ...
                    // 获取短信验证码，登陆用
                    mBoIsGetTrue = PostParma.getPhoneCode(ConnectionAddress.Base_Get_PhoneLoginCode, userName);
                    if(!mBoIsGetTrue){
                        mHandler.sendEmptyMessage(STATUS_ERROR);
                    }else {
                        mHandler.sendEmptyMessage(888);
                    }
                } else if (mIi == 2) {  // 获取短信验证码，注册用
                    mBoIsGetRegTrue = PostParma.getPhoneCode(ConnectionAddress.Base_Get_PhoneCode, userName);
                    if(!mBoIsGetRegTrue){
                        mHandler.sendEmptyMessage(STATUS_ERROR);
                    }else {
                    ...
                    mBoIsGetRegTrue = PostParma.isTruePhoneCode(ConnectionAddress.Base_IsTrue_PhoneCode, userName, login_phoneCodeI.getText().toString());
                    if(!mBoIsGetRegTrue){
                        mHandler.sendEmptyMessage(STATUS_ERROR);
                    }else{
