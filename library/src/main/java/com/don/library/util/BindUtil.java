...
import com.don.library.annotation.Resource;
        ...
        if (object instanceof Activity || object instanceof Fragment
            ...
            for (Field field : fields) {
                ...
                if (field.isAnnotationPresent(Resource.class)) {
                    ...
                    Object value = null;
                    if (color != -1) {
                        value = ContextCompat.getColor(mContext, color);
                    } else if (drawable != -1) {
                        value = ContextCompat.getDrawable(mContext, drawable);
                    } else if (string != -1) {
    ...
    private static class OnClickListener implements View.OnClickListener {
        ...
        public void onClick(View v) {
            if (mObject != null) {
                Method method = null;
                try {
                    method = mObject.getClass().getMethod(mMethodName);
                } catch (Exception e) {
                }
                if (method == null) {
                    try {
